...
arm_client.cpp
Todo: add license stuff, logging with levels, list of active signals
*/
    ...
    while (true) {
        ...
        // Create a new thread to handle the client communication
            ...
            while (!niceShutdown) {
                ...
                std::thread workerThread([receivedMsg]() { //worker thread for each high priority message. if a new message comes that is for the same thing, it will stop the old one and start a new one. look for same interface and identifier eg: can0 0x123[...]
                    //pipe command to cansend
